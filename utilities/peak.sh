...
do
    $alhena_dir/bin/alhena -o no-upseri+fi -s peak $csv_file > ${csv_file/$data_dir/$out_dir}
done 
...
if [ $is_hst = yes ]; then
    cat result.csv \
        | awk -F ',' '{printf "$alhena_dir/extra/data_printer.pl -n %s -s %s -b 20 -m ./hst/ -p $data_dir\n", \
                                    $1, $2}' \
            | sh -x
fi
